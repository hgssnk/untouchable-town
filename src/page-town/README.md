...
[遊ぶ](https://hgssnk.github.io/untouchable-town/)（[← リポジトリのREADME](../../README.md)）
...
このページは`src/index.html`（ページ一覧）には依存しない。
...
`index.html`をブラウザで開くだけで動作する（ES Modulesを使っていないので`file://`でもOK）。開発中の`live-server`の使い方は、[リポジトリのREADME](../../README.md)を参照。
...
- `window.Sim` / `window.State` / `window.createRenderer` / `window.RenderLayers` / `window.createCtx` / `window.Buttons`は、ページ単位で名前空間分けされていないグローバル。今はページ（`page-town/`）が他ページと独立しており、かつ各ページが自分の`index.html`から自分のscriptしか読み込まないので衝突しない（ページを跨いでスクリプトが同じ`window`を共有することがない）。ページが増えても、ページごとに独立したHTMLドキュメントである限りこのままで問題ない想定。
...
```math
s \to s^{\ast} \to s^{\ast} \to s^{\ast} \to \cdots
```
